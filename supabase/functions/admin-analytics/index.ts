...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [
      ...
      videosHead, videosToday, videos7, videos30, videosPending, videosApproved, videosRejected,
      shortsHead,
      ...
      admin.from('seeded_videos').select('*', { head: true, count: 'exact' }).eq('moderation_status', 'approved'),
      admin.from('seeded_videos').select('*', { head: true, count: 'exact' }).eq('moderation_status', 'rejected'),
      ...
      admin.from('property_videos').select('*', { head: true, count: 'exact' }),
      admin.from('property_videos').select('*', { head: true, count: 'exact' }).eq('is_approved', true),
      admin.from('property_videos').select('*', { head: true, count: 'exact' }).eq('is_approved', false),
    ...
    return json({
      ...
      totals: {
        ...
        videos_approved: videosApproved.count ?? 0,
        videos_rejected: videosRejected.count ?? 0,
        ...
        property_video_links: propertyVideoLinks.count ?? 0,
        property_video_links_approved: propertyVideoLinksApproved.count ?? 0,
        property_video_links_pending: propertyVideoLinksPending.count ?? 0,
